...
import { applicationsAPI, aiAPI, interviewsAPI, remindersAPI } from '../services/api.js';
import { 
  Briefcase, 
  MapPin, 
  Calendar, 
...
export const ApplicationDetails = () => {
  ...
  const [activeAITab, setActiveAITab] = useState('FOLLOWUP'); // FOLLOWUP, PROBABILITY, PREP, SALARY
  ...
  const [salaryBenchmarked, setSalaryBenchmarked] = useState(null);
  const [benchmarkingSalary, setBenchmarkingSalary] = useState(false);
  ...
  const updateStatusMutation = useMutation({
    mutationFn: (status) => applicationsAPI.update(id, { status }),
    onSuccess: () => {
      queryClient.invalidateQueries(['applicationDetails', id]);
  ...
  const addTimelineEventMutation = useMutation({
    mutationFn: (eventData) => applicationsAPI.addTimelineEvent(id, eventData),
    onSuccess: () => {
      queryClient.invalidateQueries(['applicationDetails', id]);
  ...
  const addContactMutation = useMutation({
    mutationFn: (contactsList) => applicationsAPI.update(id, { contacts: contactsList }),
    onSuccess: () => {
      queryClient.invalidateQueries(['applicationDetails', id]);
  ...
  const probAnalysis = analyses.find(a => a.type === 'INTERVIEW_PROBABILITY');
  ...
  const handleGenerateFollowUp = async () => {
    ...
    if (res.success) {
      ...
      toast.error('Failed to generate draft: ' + res.message);
    }
  };
  ...
  const handleBenchmarkSalary = async () => {
    ...
    const res = await aiAPI.analyzeSalary(offeredAmount, app.jobTitle, app.location, 'JUNIOR');
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="glass-card rounded-2xl p-6 space-y-4">
            <div className="flex justify-between items-center border-b border-white/5 pb-3">
              ...
              <button 
                ...
                className="btn btn-xs btn-primary rounded flex items-center gap-1"
              >
            ...
            <div className="relative pl-6 space-y-6 border-l-2 border-white/5">
              {[...app.timeline].reverse().map((event, idx) => (
                <div key={idx} className="relative space-y-1">
                  {/* Timeline point */}
                  <span className="absolute -left-[31px] top-1.5 w-3 h-3 rounded-full bg-primary border-4 border-[#0f172a] shadow"></span>
          ...
          <div className="glass-card rounded-2xl p-6 space-y-4">
            <div className="flex justify-between items-center border-b border-white/5 pb-3">
              <div>
                ...
                <p className="text-[10px] text-slate-400">Direct connections at {app.companyId?.name || 'Company'}</p>
              </div>
              <button 
                onClick={() => setContactModal(true)}
                className="btn btn-xs btn-outline btn-primary rounded flex items-center gap-1"
              >
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {app.contacts?.map((contact, idx) => (
                <div key={idx} className="p-3 bg-neutral/40 rounded-xl border border-white/5 flex flex-col justify-center">
                  ...
                  {contact.email && (
                    <a href={`mailto:${contact.email}`} className="text-[10px] text-primary hover:underline mt-1">
                      {contact.email}
                    </a>
                  )}
                </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          {/* AI panel box */}
          <div className="glass-card rounded-2xl p-5 border border-primary/10 space-y-4">
            ...
            <div className="tabs tabs-boxed bg-neutral/60 border border-white/5 p-1 rounded-xl grid grid-cols-4 text-[10px] font-semibold">
              <button 
                ...
                className={`tab tab-xs rounded-lg py-1.5 ${activeAITab === 'FOLLOWUP' ? 'tab-active bg-primary text-white' : 'text-slate-400'}`}
              >
                Follow-Up
              </button>
              <button 
                onClick={() => setActiveAITab('PROBABILITY')}
                className={`tab tab-xs rounded-lg py-1.5 ${activeAITab === 'PROBABILITY' ? 'tab-active bg-primary text-white' : 'text-slate-400'}`}
              >
                Probability
              </button>
              <button 
                onClick={() => setActiveAITab('PREP')}
                className={`tab tab-xs rounded-lg py-1.5 ${activeAITab === 'PREP' ? 'tab-active bg-primary text-white' : 'text-slate-400'}`}
              >
                Interview Prep
              </button>
              <button 
                onClick={() => setActiveAITab('SALARY')}
                className={`tab tab-xs rounded-lg py-1.5 ${activeAITab === 'SALARY' ? 'tab-active bg-primary text-white' : 'text-slate-400'}`}
              >
                Salary Adv
              </button>
            </div>
              ...
              {activeAITab === 'FOLLOWUP' && (
                <div className="space-y-3">
                  ...
                  <div className="form-control">
                    ...
                    <input 
                      type="text" 
                      placeholder="e.g. mention talking to David, reference project X"
                      value={customInfo}
                  ...
                  <button 
                    ...
                    className="btn btn-sm btn-primary w-full rounded-lg text-xs flex items-center justify-center gap-1.5"
                    ...
                    <div className="p-3 bg-neutral-900/60 rounded-xl border border-white/5 space-y-3">
                      <div className="flex justify-between items-center border-b border-white/5 pb-1.5">
                        ...
                        <button 
                          ...
                          className="btn btn-ghost btn-xs text-primary hover:bg-primary/10 flex items-center gap-1"
                        >
                ...
                <div className="space-y-4">
                  {probAnalysis ? (
                    <div className="space-y-4">
                      ...
                      {/* Score Calibration Factors */}
                      <div className="space-y-2">
                        <h4 className="text-[10px] uppercase font-bold text-slate-400">Scoring Factors</h4>
                        <div className="space-y-1.5">
                          {probAnalysis.factors?.map((fact, i) => (
                            <p 
                              key={i} 
                              className={`text-[10px] leading-relaxed flex items-start gap-1.5 ${fact.startsWith('✓') ? 'text-teal-400' : 'text-amber-400'}`}
                            >
                              {fact}
                            </p>
                          ))}
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="space-y-1">
                          {probAnalysis.recommendations?.map((rec, i) => (
                ...
                <div className="space-y-4">
                  <div className="bg-neutral-900/40 rounded-xl p-3 border border-white/5 space-y-2.5">
                    <h4 className="text-[10px] uppercase font-bold text-slate-400 flex items-center gap-1">
                      <HelpCircle className="w-4 h-4 text-primary" /> Expected Interview Questions
                    </h4>
              ...
              {activeAITab === 'SALARY' && (
                <div className="space-y-3">
                  ...
                  <button 
                    ...
                    className="btn btn-sm btn-outline btn-primary w-full rounded-lg text-xs"
                    ...
                    <div className="space-y-3 pt-2">
                      <div className="p-3 bg-neutral-900/60 rounded-xl border border-white/5 space-y-2">
                        ...
                        <div className="flex justify-between items-center text-[10px] border-t border-white/5 pt-1.5">
                          ...
                          <span className="font-bold text-teal-400">${salaryBenchmarked.targetSalary.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center text-[10px]">
            ...
            <div className="glass-card rounded-2xl p-5 border border-white/5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-neutral/40 border border-white/5 rounded-xl text-center space-y-1">
                  ...
                  <span className="badge badge-success badge-xs font-bold text-white">Healthy</span>
                </div>
                <div className="p-3 bg-neutral/40 border border-white/5 rounded-xl text-center space-y-1">
            ...
            <form onSubmit={handleLogSubmit} className="space-y-4">
              <div className="form-control">
                ...
                >
                  ...
                  <option value="NOTE">Other event</option>
                </select>
              </div>
              ...
              <div className="form-control">
                ...
                <input 
                  type="text"
                  placeholder="e.g. Chat with Emily, Sent portfolio note"
                  value={logTitle}
              ...
              <div className="form-control">
                ...
                <textarea 
                  placeholder="Summarize outcomes or talking points..."
                  value={logDesc}
