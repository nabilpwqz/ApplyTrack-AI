...
export const Layout = () => {
  ...
  useEffect(() => {
    fetchPendingEmails();
    // Poll every 45 seconds for new simulated recruiter emails
  ...
  const handleSyncEmails = async () => {
    ...
    if (res.success) {
      toast.success(res.message, { id: 'email-sync' });
      fetchPendingEmails();
    } else {
  ...
  const handleProcessEmail = async (eventId, action, applicationId = null) => {
    ...
    if (res.success) {
      ...
      toast.error(res.message, { id: 'email-process' });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-mesh flex flex-col">
      ...
      <header className="sticky top-0 z-40 w-full glass-panel border-b border-white/5 px-4 lg:px-8 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className="btn btn-sm btn-outline btn-primary rounded-lg flex items-center gap-2 border-white/10 hover:border-primary text-xs"
          >
          ...
          <div className="relative">
            ...
            >
              ...
              {unprocessedCount > 0 && (
                <span className="absolute top-1 right-1 w-5 h-5 rounded-full bg-error text-white font-bold text-[10px] flex items-center justify-center animate-pulse">
                  {unprocessedCount}
                </span>
              )}
            </button>
                  ...
                  {pendingEmails.length === 0 ? (
                    ...
                    pendingEmails.map((email) => (
                      <div key={email._id} className="p-3 bg-neutral/40 rounded-lg border border-white/5 space-y-2">
                        ...
                        <p className="text-xs text-slate-300 italic border-l-2 border-primary/40 pl-2 line-clamp-2">
                          "{email.bodyPreview}"
                        </p>
                        ...
                        <div className="flex gap-2 pt-1">
                          ...
                          <button 
                            ...
                            className="btn btn-xs btn-outline rounded text-slate-400 border-white/10"
                          >
                            Dismiss
                          </button>
                        </div>
          ...
          <div className="dropdown dropdown-end">
            <div tabIndex={0} role="button" className="btn btn-ghost btn-circle avatar placeholder">
              <div className="bg-primary/20 text-primary rounded-full w-9">
                <span className="text-sm font-semibold">{user?.name ? user.name[0].toUpperCase() : 'U'}</span>
              </div>
            </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-30 glass-panel border-t border-white/5 px-2 py-2 flex items-center justify-around">
        <Link to="/dashboard" className="flex flex-col items-center gap-0.5 text-slate-400 hover:text-primary">
          <LayoutDashboard className="w-5 h-5" />
          <span className="text-[10px]">Home</span>
        </Link>
        <Link to="/dashboard/applications" className="flex flex-col items-center gap-0.5 text-slate-400 hover:text-primary">
          <Briefcase className="w-5 h-5" />
          <span className="text-[10px]">Jobs</span>
        </Link>
        <Link to="/dashboard/calendar" className="flex flex-col items-center gap-0.5 text-slate-400 hover:text-primary">
          <CalendarIcon className="w-5 h-5" />
          <span className="text-[10px]">Calendar</span>
        </Link>
        <Link to="/dashboard/ai" className="flex flex-col items-center gap-0.5 text-slate-400 hover:text-primary">
          <Sparkles className="w-5 h-5 text-primary" />
          <span className="text-[10px] text-primary font-semibold">AI Hub</span>
        </Link>
        
        <div className="dropdown dropdown-top dropdown-end">
          <div tabIndex={0} role="button" className="flex flex-col items-center gap-0.5 text-slate-400 hover:text-primary">
