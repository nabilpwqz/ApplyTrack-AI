...
class AIService {
    ...
    async generateJSON(prompt, fallbackGenerator) {
        if (this.model) {
            try {
    ...
    async calculateInterviewProbability(data) {
        ...
        return this.generateJSON(prompt, () => {
            const skills = data.candidateProfile?.skills || [];
    ...
    async analyzeCompanyHealth(companyName, domain, industry) {
    ...
    async parseRecruiterEmail(emailText, sender, subject) {
