...
export const generateFollowUpEmail = async (req, res, next) => {
    try {
        ...
        if (isMongoConnected) {
            ...
            if (app) {
                companyName = app.companyId?.name || app.companyName || 'Company';
...
export const analyzeSalary = async (req, res, next) => {
    try {
