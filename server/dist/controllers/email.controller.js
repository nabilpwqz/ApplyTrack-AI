...
export const getEmailEvents = async (req, res, next) => {
    try {
        if (!isMongoConnected) {
            res.status(200).json({
                success: true,
...
export const processEmailEvent = async (req, res, next) => {
    try {
        ...
        if (action === 'CREATE') {
            ...
            emailEvent.applicationId = app._id;
            ...
            if (app) {
                if (emailEvent.extractedData.applicationStatus) {
                    app.status = emailEvent.extractedData.applicationStatus;
                }
            ...
            emailEvent.processed = true;
            emailEvent.applicationId = applicationId;
            await emailEvent.save();
        }
